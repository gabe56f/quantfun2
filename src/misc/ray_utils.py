...
__ALL__ = ["create_c2w_matrix", "calculate_rays"]
...
def create_c2w_matrix(
    azimuth_deg, elevation_deg, distance=1.0, target=np.array([0, 0, 0])
):
    """
...
def _cameras_from_opencv_projection(
    ...
    focal_pt3d = focal_length / scale
    p0_pt3d = (-principal_point - c0) / scale

    R_pt3d = R.clone().permute(0, 2, 1)
    T_pt3d = tvec.clone()
    R_pt3d[:, :, :2] *= -1
    T_pt3d[:, :2] *= -1
    ...
    cams = PerspectiveCameras(
        R=R_pt3d,
        T=T_pt3d,
        focal_length=focal_pt3d,
        principal_point=p0_pt3d,
...
def calculate_rays(
    ...
    do_normalize_cameras: bool = False,
